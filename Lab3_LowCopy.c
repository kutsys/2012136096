...
#include <unistd.h>
#include <sys/stat.h>
#include <fcntl.h> // 이미 열려있는 파일의 특성 제어를 위한 사용
#include <stdlib.h>
#include <stdio.h>
#include <string.h>

const int NAMESIZE = 50;
const int TIMEINTERVAL = 2; // 실제로 몇 초가 지난 후에 .을 찍어 ㅐ는가를 결정할 변수

int main(int argc, char *argv[]) {
	char fname_in[NAMESIZE], fname_out[NAMESIZE];
	// 각각의 복사할 파일과 복사당할 파일의 이름
	int in, out; // 파일 서술자 값을 저장하기 위한 변수
	int count = 0; // '.'를 시간을 두면서 찍어낼 카운트 변수
	char buf[64]; // 64byte씩 전송하기 위한 버퍼

	// 명령어의 인자가 3이 아닌 경우에는 조건이 맞지 않도록
	// 실행파일(LowCopy) + [in파일] + [out파일] : 3개의 인자가 필수적
	if (argc != 3) {
	...
	// strlen 함수를 이용하여 파라미터로 들어온 파일이름을 검사
	...
	while (read(in, buf, sizeof(buf)) > 0) {
		...
		if ( (time2 - time1) % TIMEINTERVAL == 1 ) {
			sleep(1); // 파일 복사 수행에 1초마다 멈추면서 한번씩 .찍기
			printf(".\n");
		}
	}
	printf("%s -> %s \n", fname_in, fname_out);
